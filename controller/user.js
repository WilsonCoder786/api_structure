...
const { addOrRemoveFollowingValidation, updateProfileValidation ,registerUserValidation, loginUserValidation, resetPasswordValidation } = require('../validations/userValidation');

const { getProperty, getPropertyForOwner } = require('../models/property')
...
exports.register = async (req, res, next) => {
    ...
    try {
        ...
        generateResponse({ user, otp: otpObj.otp }, 'OTP is sent to your email, please verify your account to login..', res);
    } catch (error) {
...
exports.login = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) return next({status:false, statusCode: STATUS_CODE.BAD_REQUEST, message: 'User not found' });
        if(user.role == "owner"){
            const isMatch = await compare(password, user.password);
            if (!isMatch) return next({status:false, statusCode: STATUS_CODE.BAD_REQUEST, message: 'Invalid credentials' });
            ...
            if (!user.is_completed) {
                ...
                return next({
                    ...
                    data: {is_completed: false, token :token}
                });
            }
            ...
            let updatedUser = await updateUserById(user._id, { $set: { device_tokens:device_token } });
            ...
            generateResponse({ User, property, token }, 'Login successful', res);
        }
        else {
            const isMatch = await compare(password, user.password);
            if (!isMatch) return next({status:false, statusCode: STATUS_CODE.BAD_REQUEST, message: 'Invalid credentials' });
...
exports.getProfile = async (req, res, next) => {
    try{
        ...
        // login successful with cookie-session
        // req.session.userId = user._id;        // req.session.email = email;
        generateResponse({ User, property }, 'Login successful', res);
...
exports.verifyToken=async(req,res,next)=>{
    try{
        ...
        if (!tokenHeader) return next({status:false, statusCode: STATUS_CODE.UNAUTHORIZED, message: 'Token header not found' });
        
        jwt.verify(tokenHeader, process.env.JWT_SECRET, async (err, decoded) => {
            if (err) {
                return next({status:false, statusCode: STATUS_CODE.UNAUTHORIZED, message: 'Invalid token' });
            }
...
exports.createAccount = async (req, res) => {
    ...
    const accountLinks = await stripe.accountLinks.create({
        ...
        return_url: `${process.env.LOCALHOST}/register${data.customer_id ? "-mobile" : ""}?account_id=${data.customer_id
            }&result=success`,
...
exports.createAccountForStripe = async (req, res) => {
    ...
    if (method === "GET") {
        ...
        const accountLinks = await stripe.accountLinks.create({
            ...
            return_url: `${process.env.LOCALHOST}/register${account.id ? "-mobile" : ""}?account_id=${account.id}&result=success`,
            type: "account_onboarding",
        })
        //   if (mobile) {
...
exports.getUserDetails = async (req, res, next) => {
    ...
    try {
        ...
        const isBlocked = await findBlockUser({ userId: userId, blockId: loginUserId });
...
exports.updateUser = async (req, res, next) => {
    ...
    try {
        ...
        if (!user.location?.coordinates?.length > 0 && (!body.lat || !body.lng)) return next({
            statusCode: STATUS_CODE.BAD_REQUEST,
...
exports.updateProfile = async (req, res, next) => {
    ...
    const { full_name, phone_number, location, facebook, instagram, userId, longitude, latitude, bio } = body;
    ...
    if(req.user.role == "owner"){
        try {
            ...
            let User = await updateUserById(req.user.id, { $set: updateFields }).populate("ssn_image profileImage")
            const token=generateToken(User)
            let property = await getPropertyForOwner({user_id:User._id})
    
            generateResponse({User,property,token}, 'Profile updated successfully', res);
        } catch (error) {
            next(new Error(error.message));
        }
    }
...
exports.fetchUsersExceptCurrentAndAdmin = async (req, res, next) => {
    ...
    try {
        const users = await searchUsersExceptCurrentAndAdmin({ page, limit, userId, q });
...
exports.activeInactiveToggle = async (req, res, next) => {
    ...
    if (!userId) return next({ statusCode: STATUS_CODE.BAD_REQUEST, message: 'User id is required' });

    try {
        const updatedUser = await updateUserById(userId, { $set: { isActive } });
        if (!updatedUser) return next({ statusCode: STATUS_CODE.BAD_REQUEST, message: 'User not found' });
