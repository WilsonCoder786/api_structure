...
exports.fetchProperties = async (req, res, next) => {
  ...
  try {
    ...
    generateResponse(users, "Properties fetched successfully", res);
  } catch (error) {
    next(new Error(error.message));
  }
};
...
exports.getAllPropertiesByUserId = async (req, res, next) => {
  try {
    ...
    generateResponse(properties, "Properties retrieved successfully", res);
  } catch (error) {
    next(new Error(error.message));
  }
};
...
exports.getHomeScreenPropertiesForTenant = async (req, res, next) => {
  try {
      ...
      let propertyListSecond = propertyListFirst
          let responseData;
      switch (status) {
          case "topRated":
...
exports.filterProperties = async (req, res, next) => {
  try {
    ...
    const { propertyType, size, budget, bedroomsCount, bathroomsCount, latitude, longitude, radius} = req.body;
    ...
    // Execute the query
    const properties = await PropertyModel.find(query).populate({
      path: 'user_id',
      populate: {
    ...
    if (properties.length === 0) {
      generateResponse(properties, "No properties found. Try removing some filters.", res);
    } else {
